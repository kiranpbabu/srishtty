...
odoo.define("web_m2x_options.web_m2x_options", function (require) {
    ...
    FieldMany2One.include({
        ...
        init: function () {
            this._super.apply(this, arguments);
            if (!_.isUndefined(ir_options["web_m2x_options.noOpen"])) {
                if (!is_option_set(ir_options["web_m2x_options.noOpen"])) {
                    this.noOpen = true;
                }
            }
        },
        ...
        _searchCreatePopup: function (view, ids, context, dynamicFilters) {
            var options = this._getSearchCreatePopupOptions(
                view,
                ids,
                context,
                dynamicFilters
            );
            if (!_.isUndefined(ir_options["web_m2x_options.noCreate"])) {
                if (!is_option_set(ir_options["web_m2x_options.noCreate"])) {
