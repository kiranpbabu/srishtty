...
class ScssEditor(models.AbstractModel):
    ...
    def _get_custom_attachment(self, url):
        return (
            ...
            .with_context(bin_size=False, bin_size_datas=False)
    ...
    def _replace_variables(self, content, variables):
        for variable in variables:
            variable_content = "{}: {};".format(variable["name"], variable["value"])
            regex = r"{}\:?\s(.*?);".format(variable["name"])
    ...
    def replace_content(self, url, xmlid, content):
        ...
        if not custom_view.exists():
            view_to_xpath = (
                self.env["ir.ui.view"]
                .get_related_views(xmlid, bundles=True)
    ...
    def replace_values(self, url, xmlid, variables):
        content = self._replace_variables(self.get_content(url, xmlid), variables)
